...
locals {
  decoded_manifests = flatten([
    for file in local.manifest_files :
    lookup(local.manifests_to_deploy, file, false) == true ?
...
# Output the list of manifests that were enabled
output "enabled_manifests" {
  value = [for file, enabled in local.manifests_to_deploy : file if enabled]
}
